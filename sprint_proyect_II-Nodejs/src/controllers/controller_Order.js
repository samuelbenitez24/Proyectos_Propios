...
exports.get_record = async (req, res) => {
  ...
  try {
    ...
    //verifica si hay lista de historial de pedido de cliente 
    if(userFind.order.length===0) throw new Error
...
exports.edit_status_order = async (req, res) => {
  ...
  try {
    ...
    //llamar al usuario
    const user=await UserModels.findById({_id:Ord_list.usuario});
    ...
    if (Ord_list.state.equals(pendiente._id)) {
      ...
      res.status(200).json({msg:'El estado del pedido del usuario '+user.usuario+" es: "+confirmado.name});
      
    }else if (Ord_list.state.equals(confirmado._id)) {
  
      Ord_list.state=enPreparacion._id;
      res.status(200).json({msg:'El estado del pedido del usuario '+user.usuario+" es: "+enPreparacion.name});
  
    }else if (Ord_list.state.equals(enPreparacion._id)) {
  
      Ord_list.state=enviado._id;
      res.status(200).json({msg:'El estado del pedido del usuario '+user.usuario+" es: "+enviado.name});
  
    }else if (Ord_list.state.equals(enviado._id)) {
  
      Ord_list.state=entregado.id;
      res.status(200).json({msg:'El estado del pedido del usuario '+user.usuario+" es: "+entregado.name});
    }
